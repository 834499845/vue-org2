            ...
            <ul>
              <li class="signDetailsListsLi" :key="index" v-for="(item,index) in signDetailsLists" v-show="signDetailsLists.length > 0">
                <div class="signDetailsListsTop">
                  <div>{{item.userId}}</div>
                  <div>{{item.createTime | timeFun}}</div>
                </div>
                <div class="signDetailsListsCenter">
                  ...
                  <div>
                    {{item.addressTitle}} {{item.address}}
                  </div>
                </div>
                <div class="signDetailsListsCenter">
                  <div>
  ...
  methods: {
    ...
    signDetailsClick(index, itemArrindex) {
      ...
      if (self.rightLists[index].arr[itemArrindex].signCount * 1 > 0) {
        self
          .getsignWebSignOutlist(JSON.stringify(obj))
          .then(data => {
    ...
    searchClick() {
      ...
      self
        .getsignWebSignOutStatistics(object)
        .then(data => {
          if (data.statusCode == 200) {
            self.dataLists = [];
    ...
    ...mapActions([
      "getsignWebSignOutlist",
      "getsignWebSignOutStatistics",
      "getsignWebSignOutExport"
    ])
  },
  filters: {
